...
highest_rating = st.session_state.manager.highest_rating()
lowest_rating = st.session_state.manager.lowest_rating()
highest_rated_movies = st.session_state.manager.highest_rated_movies()
lowest_rated_movies = st.session_state.manager.lowest_rated_movies()
st.title("Here are the current PopcornPicker statistics 📈")
col1, col2 = st.columns(2)
with col1:
    st.subheader(f"Average Rating: {st.session_state.manager.calc_avg():.2f}")
    ...
    st.write("")
with col2:
    st.subheader(f"Mean Rating: {st.session_state.manager.calc_mean()}")
